...
public sealed partial class DriversViewModel : ObservableObject
{
    ...
    private async Task InstallSelectedAsync()
    {
        ...
        WeakReferenceMessenger.Default.Send(new InstallBatchStartedMessage(L.T("NavDrivers"), selectedUpdates));
    ...
    public async Task InstallItemsAsync(IReadOnlyList<UpdateItem> items) =>
        await RunInstallBatchAsync(items.Cast<DriverUpdateItem>().ToList());

    private async Task RunInstallBatchAsync(List<DriverUpdateItem> selectedUpdates)
    {
        ResetCancellation();
    ...
    private void OnInstallProgressReported(DriverUpdateItem update, int percent)
    {
